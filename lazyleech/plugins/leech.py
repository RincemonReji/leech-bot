...
from .. import ADMIN_CHATS, ALL_CHATS, PROGRESS_UPDATE_DELAY, session, help_dict, LEECH_TIMEOUT, MAGNET_TIMEOUT
from ..utils.aria2 import aria2_add_torrent, aria2_tell_status, aria2_remove, aria2_add_magnet, Aria2Error, aria2_tell_active, is_gid_owner
...
@Client.on_message(filters.command(['torrent', 'ziptorrent']) & filters.chat(ALL_CHATS))
async def torrent_cmd(client, message):
    text = message.text.split(' ', 1)
    send_as_zip = 'zip' in text.pop(0)
    link = None
...
- /ziptorrent <i>(as reply to a Torrent URL or File)</i>''')
        return
    await initiate_torrent(client, message, link, send_as_zip)
...
@Client.on_message(filters.command(['magnet', 'zipmagnet']) & filters.chat(ALL_CHATS))
...
- /zipmagnet <i>(as reply to a Magnet URL)</i>''')
        return
    await initiate_magnet(client, message, link, send_as_zip)
...
leech_statuses = dict()
async def handle_leech(client, message, gid, reply, user_id, send_as_zip):
    prevtext = None
    ...
    while torrent_info['status'] in ('active', 'waiting', 'paused'):
        ...
        seeders = torrent_info['numSeeders']
        peers = torrent_info['connections']
...
<b>Peers:</b> {peers}'''
    ...
    if torrent_info['status'] == 'error':
        ...
        upload_queue.put_nowait((client, message, reply, torrent_info, user_id, send_as_zip))
        try:
