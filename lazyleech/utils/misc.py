...
async def split_files(filename, destination_dir):
    ...
    if (await get_file_mimetype(filename)).startswith('video/'):
        ...
        if 'duration' in video_info:
            ...
            while duration > ss:
                ...
                video_info = (await get_video_info(filepath))['format']
            ...
            return files
    args = ['split', '--verbose', '--numeric-suffixes=1', '--bytes=1950M', '--suffix-length=2']
    if ext:
    ...
    stdout, _ = await proc.communicate()
    return [i[15:-1] for i in stdout.decode().strip().split('\n')]
